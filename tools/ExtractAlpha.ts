...
program
  .name('extract-alpha')
  .description(
    'Restore alpha channel from two images on white and black backgrounds',
  )
  .version('1.0.0');

// program will have three required arguments: white image path, black image path, output path
program
  .argument('<img-on-white>', 'Path to image rendered on white background')
  .argument('<img-on-black>', 'Path to image rendered on black background')
  .argument('<output>', 'Path to output image with alpha channel restored')
  .action(async (imgOnWhite: string, imgOnBlack: string, output: string) => {
    try {
      await extractAlphaTwoPass(imgOnWhite, imgOnBlack, output);
      console.log(`Alpha channel extracted and saved to ${output}`);
    } catch (error) {
      console.error('Error extracting alpha channel:', error);
    }
  });
