...
import type { AppConfigMap } from "@/types/AppConfig";
import type { FortunePoemContentType } from "@/types/FortunePoems";
import {
  KaucimStoriesPackType,
  KaucimStoryLineType,
} from "@/types/KaucimStories";
...
import appConfigRaw from "@/masterdata/appConfig.json";
...
type MasterDataContextType = {
  ...
  getAppConfigValue: (key: string) => string | undefined;
...
const LOCAL_APP_CONFIG_MAP: AppConfigMap =
  appConfigRaw as unknown as AppConfigMap;
...
export function MasterDataProvider({
  ...
  const [kaucimStories] = useState<KaucimStoriesPackType>(
    LOCAL_KAUCIM_STORIES_MAP,
  );
  const [appConfig] = useState<AppConfigMap>(LOCAL_APP_CONFIG_MAP);
  ...
  const getAppConfigValue = useCallback(
    (key: string) => appConfig[key],
    [appConfig],
  );

  const getKaucimStoryBundle = useCallback(
    (concern: KAUCIM_CONCERNS, language: LanguageKey, stickNumber: number) => {
  ...
  const value: MasterDataContextType = useMemo(
    () => ({
      manifest,
      localManifest,
      appConfig,
